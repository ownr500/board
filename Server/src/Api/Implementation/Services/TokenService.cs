...
public class TokenService : ITokenService
{
    ...
    public async Task<List<RevokedTokenModel>> GetRevokedTokensAsync(CancellationToken none)
    {
        var result = await _dbContext.RevokedTokens
            .ToListAsync(none);
        return result.Select(x => new RevokedTokenModel(
                x.Token, x.TokenExpireAt
            )
        ).ToList();
    }
